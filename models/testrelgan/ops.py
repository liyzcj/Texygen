...
def get_losses(generator, discriminator, gan_type):
    ...
    if gan_type == 'standard':  # the non-satuating GAN loss
        ...
        # for print sig log
        steps_sig = []
        nsteps = len(discriminator.splited_steps)
        sig_real = tf.reshape(tf.nn.sigmoid(d_out_real), [nsteps,-1]) #[nsteps, x]
        sig_fake = tf.reshape(tf.nn.sigmoid(d_out_fake), [nsteps,-1]) #[nsteps, x]
        for i in range(nsteps):
            out_real = tf.reduce_mean(sig_real[i :(i+1)], name=f"Sig{i+1}")
            out_fake = tf.reduce_mean(sig_fake[i :(i+1)], name=f"Sig{i+1}")
            steps_sig.append([out_real, out_fake])
        ...
        steps_sig = []
        nsteps = len(discriminator.splited_steps)
        sig = tf.reshape(tf.nn.sigmoid(d_out_real - d_out_fake), [nsteps,-1]) #[nsteps, x]
        for i in range(nsteps):
            out = tf.reduce_mean(sig[i :(i+1)], name=f"Sig{i+1}")
            steps_sig.append(out)
    ...
    return log_pg, g_loss, d_loss, steps_sig
