...
from models.testrelgan.ops import get_losses
...
class MRelgan(Gan):
    ...
    def init_real_training(self, data_loc):
        ...
        # Get losses
        log_pg, g_loss, d_loss, self.steps_sig = get_losses(
    ...
    def train_real(self, data_loc=None):
        ...
        for _ in range(self.nadv_steps):
            ...
            for _ in range(self.gsteps):
                sig_out = self.generator.train(
                    self.sess, self.gen_data_loader.random_batch(), self.steps_sig)
            ...
            g_loss_np, d_loss_np, test_out = self.sess.run(
                [self.generator.loss, self.discriminator.loss, self.discriminator.test], feed_dict=feed)
            print(test_out[0][4, :, 23, 22])
            # update global step
            self.sess.run(self.global_step_op)

            print(f"Epoch: {niter} G-loss: {g_loss_np:.4f} D-loss: {d_loss_np:.4f}  Time: {toc-tic:.1f}s")
            print("Sigmoid: ",  sig_out)
