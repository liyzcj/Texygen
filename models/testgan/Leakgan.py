...
from models.testgan.LeakganDataLoader import DataLoader, DisDataloader
from models.testgan.LeakganDiscriminator import Discriminator
from models.testgan.LeakganGenerator import Generator
from models.testgan.LeakganReward import Reward
...
class Testgan(Gan):
    ...
    def init_real_trainng(self, data_loc=None):
        ...
        discriminator = Discriminator(sequence_length=self.sequence_length, num_classes=2, vocab_size=self.vocab_size,
                                      ...
                                      batch_size=self.batch_size, hidden_dim=self.hidden_dim,
                                      start_token=self.start_token,
                                      goal_out_size=goal_out_size, step_size=1,
    ...
    def train_real(self, data_loc=None):
        ...
        if FLAGS.restore:
            restore_model = tf.train.latest_checkpoint(self.save_path)
            print(f"Restore from:{restore_model}")
            ...
            for epoch_ in range(self.adversarial_epoch_adv):
                ...
                for index in range(1):
                    ...
                    _, _, g_loss, w_loss = self.sess.run(
                        [self.generator.manager_updates, self.generator.worker_updates, self.generator.goal_loss,
                         self.generator.worker_loss, ], feed_dict=feed)
                    print(f"epoch {epoch} \t g_loss: {g_loss:.3f} w_loss: {w_loss:.3f} rewards: {np.mean(rewards):.3f}")
                end = time()
                self.add_epoch()
                print(f"adv-G(global epoch:{self.epoch}): epoch:{epoch}--{epoch_} \t time: {end - start:.1f}s")
                if epoch_ % 1 == 0 or epoch == self.adversarial_epoch_num - 1:
                    generate_samples_gen(self.sess, self.generator, self.batch_size, self.generate_num, self.generator_file)
                    get_real_test_file()
                    self.evaluate()

                start = time()
                for epoch__ in range(10):
                    print(f"adv-D: epoch:{epoch}--{epoch_}: " + '>'*epoch__ + f"({epoch__}/10)", end='\r')
                    dloss = self.train_discriminator()
                end = time()
