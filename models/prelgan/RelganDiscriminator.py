...
class Discriminator(Dis):
    ...
    def logits(self, x_onehot):
        ...
        for filter_size, num_filter in zip(filter_sizes, num_filters):
            ...
            with tf.name_scope("multi_poll"):
                total_polled = []
                for length in self.splited_steps:
                    slice_out = tf.strided_slice(out, [0,0,0,0], [batch_size, length-filter_size+1, dis_emb_dim, num_filter], name="stride")
                    pooled = tf.nn.max_pool(slice_out, ksize=[1, length - filter_size + 1, 1, 1],
                                            strides=[1, 1, 1, 1], padding='VALID',
                                            name="pool")  # batch_size x 1 x num_rep x num_filter
                    total_polled.append(pooled)
                total_polled = tf.concat(total_polled, 0)
            pooled_outputs.append(total_polled)
